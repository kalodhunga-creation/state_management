...
import 'package:state_management/models/observable_action.dart';
...
class Observable<T> {
  ...
  T get value {
    if (!_hasValue) {
      throw Exception(
          '[PMVVM] Observable has no value. You should check if observable.hasValue '
  ...
  Stream<T> get stream =>
      _streamController.stream.map((action) => action.value);
  ...
  void _addAction(ObservableAction<T> action) {
    ...
    if (MVVMConfig.trackObservablesHistory) _history.add(action);
